...
const RetakeRequestsPage = () => {
    ...
    const fetchRequests = async () => {
        ...
        try {
            ...
            const { data } = await api.get('/api/retakes', { params });
            setRequests(data);
        } catch (error) {
            toast.error("Failed to fetch retake requests.");
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleUpdateRequest = async (requestId, status) => {
        try {
            await api.put(`/api/retakes/${requestId}`, { status });
                    ...
                    {requests.map(req => (
                        <div key={req._id} className="bg-surface p-4 rounded-lg shadow-md space-y-3">
                            <div>
                                ...
                                <p className="text-sm text-slate-600">{req.exam.title}</p>
                            </div>
                        ...
                        <tbody>
                            {requests.map(req => (
                                <tr key={req._id} className="border-b border-slate-100 hover:bg-slate-50">
                                    ...
                                    <td className="p-4 text-slate-600">{req.exam.title}</td>
